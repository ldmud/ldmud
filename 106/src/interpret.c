...
#  define CLEAN_CSP \
   ...
   */
     ...
     */
printf("DEBUG: call_lambda() csp %p curobj %p %s prog %p\n", csp, current_object, current_object->name, current_object->prog);
...
printf("DEBUG:  (s)efun called with curobj %p\n", current_object);
        /* Can't call from a destructed object */
        if (current_object->flags & O_DESTRUCTED)
        {
            /* inter_sp == sp */
...
printf("DEBUG: dump_trace(%d) curprog %p csp %p\n", how, current_prog, csp);
    if (!current_prog)
        return NULL;
     ...
     */
    p = &control_stack[0];
printf("DEBUG: dump starts at %p (top %p)\n", p, csp);
    do {
...
printf("DEBUG: dump %p (top %p): extern %d ob %p %s prog %p pc %p\n", p, csp, csp->extern_call, ob, ob ? ob->name : "", prog, dump_pc);
...
not_catch:  /* The frame does not point at a catch here */
        ...
        if (p[0].funstart == EFUN_FUNSTART)
        {
            ...
            name = instrs[p[0].instruction].name;
            if (name)
            {
                printf("#\'%-9s bound to '%20s' ('%20s')\n"
                  , name, ob->prog->name, ob->name);
                debug_message("#\'%-9s bound to '%20s' ('%20s')\n"
                  , name, ob->prog->name, ob->name);
            }
            else
            {
                printf("<efun closure %d> bound to '%20s' ('%20s')\n",
                    p[0].instruction, ob->prog->name, ob->name);
                debug_message("<efun closure %d> bound to '%20s' ('%20s')\n",
                    p[0].instruction, ob->prog->name, ob->name);
            }
            continue;
        }
