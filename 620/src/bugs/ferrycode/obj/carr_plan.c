...
void reset(int iArg) {
  ...
  ::reset();
  if(iArg) return;
  set_name("carrack deck plan");
  set_alt_name("deck plan");
  set_alias("plan");
  add_alias("plans");
  set_short("deck plan for the Carrack-class ship");
  set_long("\
This is a small scroll of paper detailing the arrangement of rooms\n\
in the Carrack class vessel. You should \"read\" it to view it.\n");
  set_value(0);
  set_no_sell();
}
