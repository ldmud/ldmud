 ...
 *
 *---------------------------------------------------------------------------
 * The lexer uses combined file-input/macro-expansion buffer (TODO: TRUE),
 * called defbuf[]. Within this buffer, MAXLINE bytes are reserved as
 * file-input buffer, its beginning and end marked with the pointers
 * linebufstart and linebufend. In this space, pointer outp points to the
 ...
 * the next MAXLINE bytes are read into the buffer starting at linebufstart.
 *
 ...
 * necessary as compilation and thus lexing will end with the EOF character).
 *---------------------------------------------------------------------------
 */
...
/* TODO: BOOL */ int pragma_strict_types;
...
char *last_lex_string;
  /* The (shared) string making up the lexem recognized last.
   */
...
static char *defbuf = NULL;
  /* The combined input/expansion buffer (TODO: really expansion?)
   */
...
static struct ident *ident_table[ITABLE_SIZE];
  /* The table of identifiers, hashed of the identifier name.
   * See lex.h for a more explanation.
   */
...

{
    ...
    add_permanent_define("LPC3", -1, string_copy(""), 0);
        ...
        add_permanent_define("__COMPAT_MODE__", -1, string_copy(""), 0);
#endif
#ifdef NATIVE_MODE
        add_permanent_define("__NATIVE_MODE__", -1, string_copy(""), 0);
#endif
#ifdef EUIDS
        add_permanent_define("__EUIDS__", -1, string_copy(""), 0);
#endif
    mtext[0] = '"';
    mtext[1] = '/';
    ...
    add_permanent_define("__ERQ_MAX_SEND__", -1, string_copy(mtext), 0);
    sprintf(mtext, "%d", ERQ_MAX_REPLY);
    add_permanent_define("__ERQ_MAX_REPLY__", -1, string_copy(mtext), 0);
#endif
