...
#ifdef MAX_TIME
        p->save_watchdog_state = suspend_watchdog;
#endif
...
#ifdef MAX_TIME
    suspend_watchdog = p->save_watchdog_state;
#endif
    error_recovery_pointer = p->recovery_info.last;
    xfree ((char *)p);
    return sp;
...
#ifdef MAX_TIME
            if (max_time > 0 && eval_time >= 0)
            {
                printf("Evaluation too long: %d seconds\n", eval_time + max_time);
            }
            else
#endif
    ...
    if (master_ob == 0 || master_ob->flags & O_DESTRUCTED) {
        ...
        if (inside || !master_ob) {
            if (destructed_master_ob) {
                ...
                if (obj_list)
                {
                    obj_list_end->next_all = ob;
                    obj_list_end = ob;
                }
                else
                {
                    obj_list = obj_list_end = ob;
                }
                ob->next_all = NULL;
#else
