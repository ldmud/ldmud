...
special_lvalue;
  /* When assigning to vector and string ranges, the target information
 ...
 *  - a svalue structure into which the assignment is done in the first
 *    place (this structure is always first so that the protector
 *    structures can be used instead of normal svalues),
 *  - the necessary information to store the assigned svalue into its
 *    place in the actual target,
 *  - a protective reference to the actual target.
 *
 ...
 * TODO:: However, passing vector ranges as ref-parameters to functions
...
struct protected_lvalue
{
    struct svalue v;
       ...
       */
    struct svalue protector; /* protects .v.u.lvalue */
};
...
/* Put a duplicate of svalue <to> into svalue <from>, meaning that the original
...
/* TODO: All the stack handling functions should go into a separate
 * TODO:: module 'vm-stack', with the inlines defined in 'vm-stack-i.c'
 * TODO:: This way functions like m_indices_filter() can go into their
 * TODO:: respective modules and still take advantage of inlining.
 */
