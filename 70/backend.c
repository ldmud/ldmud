...
printf("DEBUG: process(): restart after error\n");
    ...
    while (NULL != (obj = obj_list)
        && (bAll || !comm_time_to_call_heart_beat)
...
#ifdef DEBUG_RESET
printf("DEBUG: process: loop: %ld/%ld: obj %p '%s'\n", num_last_processed, num_listed_objs, obj, obj ? obj->name : "<null>");
#endif
...
printf("DEBUG: process: loop end: %ld: obj %p '%s'\n"
    ...
    if (last_time != current_time)
    {
        ...
        /* Heartbeat stats */
        if (hb_num_done > num_hb_objs) /* happens when heartbeats turn themselves off */
            avg_num_hb_done += num_hb_objs - (avg_num_hb_done >> 10);
        else
