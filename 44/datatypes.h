 ...
 * It is allocated big enough for chains[] to hold all (.mask+1) chain heads.
 * TODO: Document its fields 
 */
...
struct hash_mapping {
    ...
    struct map_chain *deleted;
      /* Protector mappings only: list of deleted entries, but kept pending
       * because the they may still be destination of an lvalue
       */
...
struct condensed_mapping {
    ...
    /* struct svalue misc[ ... ];
     *
     *   The non-string keys and their associated values.
     *   For entry number 'x' (and num values per key), the association
     *   is:
     *     key    -> misc[(num+1) * x]
     *     values -> misc[(num+1) * x + 1 .. (num+1) * x + num]
     * TODO: Or is the layout as for the string keys?
     */

    p_int misc_size;    /* Total size of the non-string key values in byte(!) */
     ...
     *   The association is (num values per key):
     *
     *     string[x] -> s_values[num * x .. num * x + num - 1]
     *
     *   Note that the value pointers for invalidated string keys
     *   are still valid.
     */
};
...
#define CM_MISC(m) ((struct svalue *)(m))
   ...
   * after the last non-string key in that mapping.
   */
