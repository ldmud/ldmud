    ...
    SOCKET_T new_socket;
{
	...
	abort();
        ...
        perror("ignored error: fcntl socket FNDELAY");
#endif
    }
    ...
    char *buff;
{
	...
	if (NextCmdGiver < 0) {
	  ...
	  for (i = 0; i < numports; i++) {
		...
		if (res == -1) {
		    /* BeOS returns errno -1 instead of EINTR :-( */
	 ...
	 * include the descriptor number in the set to be selected on.
         * Note for BeOS: reading a non-ready UDP socket is a bad thing.
         *   In PR2 it caused the thread to freeze, in R4 it blocks all
         *   other sockets from accepting input.
	 */
#ifndef __BEOS__
	if (udp_s >= 0)
#else
    ...
    struct svalue *sp;
{
	...
	if ( NULL != (ip->quote_iac = sp->u.number) ) {
