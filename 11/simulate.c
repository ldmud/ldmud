...
fprintf(stderr, "DEBUG: load_object('%s', %d, %d)\n", lname, dont_reset, depth);
#ifdef DEBUG
    if ('/' == lname[0])
    ...
    while (MY_TRUE) {
fprintf(stderr, "DEBUG: compile-loop: '%s' for '%s'\n", fname, name); fflush(stderr);
...
fprintf(stderr, "DEBUG:   inheriting '%s'\n", inherit_file);
...
fprintf(stderr, "DEBUG: compiled '%s' for '%s'\n", fname, name);
    if (num_parse_error > 0) {
	error("Error in loading object\n");
    }
...
fprintf(stderr, "DEBUG: lookfor_object(%s, %d)\n", str, bLoad);
