    ...
    char *buff;
{
    if (*buff == 0 || strcmp(buff, "tables") == 0) {
        ...
        if (verbose)
        {
            ...
            add_message("Objects in list:\t\t\t%8ld   (%ld destructed)\n"
                       , (long)num_listed_objs, (long)new_destructed);
            add_message("Objects processed in last cycle:\t%8ld   (%5.1f%%)\n"
                       , (long)num_last_processed
                       , (float)num_last_processed / (float)num_listed_objs * 100.0);
            sprintf(print_buff, "%10.1f", avg_last_processed);
            add_message("  on average:\t\t\t\t%s (%5.1f%%)\n"
                       , print_buff
                       , (float)avg_last_processed / (float)num_listed_objs * 100.0
                       );
        }
#endif
