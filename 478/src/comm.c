...
/*-------------------------------------------------------------------------*/
mp_int
add_binary_message (const char * msg, size_t size, int flags)

/* Add a binary (ie. possibly containing 0s) message <msg> of length <size>
 * to the output. Result is the number of characters written.
 * This function is used for the efun binary_message() as well as internally
 * to send telnet negotiations.
 *
 * Flag settings are interpreted bitwise and may be ored
 * together:
 *
 *   Bit 0 (ADD_BINARY_AS_MESSAGE): when set, add_message() is used
 *     instead of write(). Thus no previous flushing of the buffer is needed,
 *     but the output is not immediate, nor can the number of bytes actually
 *     sent be determined - the return value is undefined.
 *   Bit 1 (ADD_BINARY_FLUSH_MESSAGE): The buffer is flushed _after_ adding
 *     the message. Useful only in conjunction with Bit 0.
 *
...

{
     ...
     && !ip->do_close)
    {
        ...
        if (flags & ADD_BINARY_AS_MESSAGE)
        {
            /* Write before flush... */

            wrote = size;

            save_stc = sending_telnet_command;
            sending_telnet_command = MY_TRUE; /* turn off IAC quoting */
            ...
            sending_telnet_command = save_stc;

            if (flags & ADD_BINARY_FLUSH_MESSAGE)
                add_message(message_flush);
        }
        else
        {
            ...
            for (i = 6;;) {
#ifdef USE_PTHREAD
                wrote = (mp_int)thread_socket_write(ip->socket, (char *)msg
                                                   , (size_t)size, ip);
#else
                wrote = (mp_int)socket_write(ip->socket, msg, (size_t)size);
#endif
                if (wrote != -1)
                    break;
                switch(errno)
...

{
    ...
    msg[2] = option;
    SEND_TELNET_COMMAND(
      add_binary_message(msg, 3, ADD_BINARY_AS_MESSAGE|ADD_BINARY_FLUSH_MESSAGE);
    )
} /* send_telnet_option() */
...

{
    ...
    flags = ((sp->u.number & 1) ? ADD_BINARY_AS_MESSAGE : 0)
          | ((sp->u.number & 2) ? ADD_BINARY_FLUSH_MESSAGE : 0)
          ;

    /* Send the message */
    wrote = add_binary_message(message, size, flags);
