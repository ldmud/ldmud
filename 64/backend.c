 ...
 *
 * If MAX_TIME is defined, the backend also implements a watchdog counter
 * 'eval_time', which is incremented by every alarm. The evaluator monitors
 * this variable and aborts execution when it reaches 0. Parts of the driver
 * which are expected to take some time (compiling objects on a slow machine
 * for example) can freeze the watchdog for some time with the macros
...
#ifndef OLD_RESET
static void look_for_objects_to_reset(void);
#endif
...
#ifdef MAX_TIME
ALARM_HANDLER(catch_alarm, comm_time_to_call_heart_beat = 1; total_alarms++; if (!suspend_watchdog) eval_time += ALARM_TIME; alarm(ALARM_TIME); )
#else
 ...
 * in clean_up() won't mess up the handling.
 */
...
#else
        /* Check if a call to reset() is due (just needing a reset is
         * not sufficient). If that is the case, the object will not be
         * cleant up.
         * We do nothing about the actual reset itself.
         */
        if (ob->time_reset 
         && ob->time_reset < current_time 
         && !(ob->flags & (O_SWAPPED|O_RESET_STATE))
           )
        {
            NOOP;
        }
...
static void
look_for_objects_to_reset(void)

/* Reset all objects which are now due, but don't take more than
 * one heartbeat time for it.
 *
 * This function must not be called before look_for_objects_to_swap().
 * Changes in what objects qualify for a reset() _call_ must match
 * the corresponding test in look_for_objects_to_swap().
 *
 * The function maintains its own error recovery info so that errors
 * in one reset() won't stop the whole process or forget to reinsert
 * the object into the reset table.
 */

{
    struct object *obj;
      /* Object currently reset - static so errors won't clobber it.
       */
    struct error_recovery_info error_recovery_info;
      /* Local error recovery info */
     ...
     */
    alarm(ALARM_TIME); /* TODO: This shouldn't be necessary */

    /* Setup the error recovery handler.
     */
    error_recovery_info.last = error_recovery_pointer;
    error_recovery_info.type = ERROR_RECOVERY_BACKEND;
    error_recovery_pointer = &error_recovery_info;
    ...
    {
        clear_state();
        debug_message("Error in look_for_objects_to_reset.\n");
    }

    while (!comm_time_to_call_heart_beat 
       &&  NULL != (obj = rtable_next_due(current_time))
          )
    {
        /* Objects which have not been touched
         * since the last reset just get a new due time set. Objects which
         * are not swapped in are not reset (the swapper will assign a new
         * due-time on swap-in).
         */
        ...
        {
#if 0
printf("DEBUG: %ld RESET '%s'%s due %ld\n"
      , current_time, obj->name, (obj->flags & O_RESET_STATE) ? " (reset)" : ""
      , obj->time_reset
      );
#endif
...
#if 0
else printf("DEBUG: %ld RESET swapped '%s'%s due %ld\n"
      , current_time, obj->name, (obj->flags & O_RESET_STATE) ? " (reset)" : ""
      , obj->time_reset
      );
#endif
 ...
 *
 * Important: this function sets the current_time variable.
 */

{
     ...
     * the SIGALRM handler here.
     */
    alarm(ALARM_TIME); /* TODO: This shouldn't be necessary (but needs to be done one
                        * TODO:: at the beginning of the backend loop
                        */
