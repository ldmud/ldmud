...
/* Scan through all objects and test if they are destructed, need to be
 * reset, cleaned up and/or swapped. If <bAll> is false (the normal case),
 * the scan will run at max one ALARM_TIME period, then all processed
 * objects are moved to the end of the list so that the next call will
 * continue with the yet unprocessed objects.
 *
 * If an object is flagged as 'destructed', it is removed from the
 * object list and really deallocated.
 *
