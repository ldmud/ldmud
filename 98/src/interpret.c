 ...
 *    the instruction at the returned address.
 *
 *---------------------------------------------------------------------------
 */
#if 0
 ...
 * TODO: Put these values into an include file for the mudlib.
 */
...
static /* TODO: opcode */ char **break_sp;
...
int function_index_offset;
   ...
   * TODO: Is this true? Compare with the calls of setup_new_frame1().
   */
...
struct svalue catch_value = { T_INVALID } ;
  /* Holds the value throw()n from within a catch().
   */
...
static struct control_stack *csp;
   ...
   * values as it will terminate execution.
   */
...
static int apply_low(char *, struct object *, int, short /* TODO: BOOL */);
static int inter_sscanf(int, struct svalue *);
static struct vector *inter_add_array(struct vector *, struct vector**);
static void call_simul_efun(int code, struct object *ob, int num_arg);

/*-------------------------------------------------------------------------*/
/* Auxiliaries for F_TERMINAL_COLOUR */
...
static /* TODO: BOOL */ int at_end(int i, int imax, int z, int *lens)
...
static void assign_string_range(struct svalue *source, /* TODO: BOOL */ int do_free);
static void assign_protected_string_range(
    struct protected_range_lvalue *dest,struct svalue *source, /* TODO: BOOL */ int do_free);
...
assign_checked_svalue_no_free (struct svalue *to, struct svalue *from
                              , struct svalue *sp, /* TODO: opcode */ char *pc
                              )
 ...
 * assignment. Structured values will necessiate to do the assignment before
...
/*-------------------------------------------------------------------------*/
static void
assign_string_range (struct svalue *source, /* TODO: BOOL */ int do_free)
...
/*-------------------------------------------------------------------------*/
static void
stack_overflow (struct svalue *sp, struct svalue *fp, /* TODO: opcode */ char *pc)
...
/*-------------------------------------------------------------------------*/
static INLINE struct svalue *
push_indexed_lvalue (struct svalue *sp, /* TODO: opcode */ char *pc)
...
/*-------------------------------------------------------------------------*/
static INLINE struct svalue *
push_rindexed_lvalue (struct svalue *sp, /* TODO: opcode */ char *pc)
...
/*-------------------------------------------------------------------------*/
static INLINE struct svalue *
push_protected_indexed_lvalue (struct svalue *sp, /* TODO: opcode */ char *pc)
...
/*-------------------------------------------------------------------------*/
static INLINE struct svalue *
push_protected_rindexed_lvalue (struct svalue *sp, /* TODO: opcode */ char *pc)
...
/*-------------------------------------------------------------------------*/
static INLINE struct svalue *
index_lvalue (struct svalue *sp, /* TODO: opcode */ char *pc)
...
/*-------------------------------------------------------------------------*/
static INLINE struct svalue *
rindex_lvalue (struct svalue *sp, /* TODO: opcode */ char *pc)
...
/*-------------------------------------------------------------------------*/
static INLINE struct svalue *
protected_index_lvalue (struct svalue *sp, /* TODO: opcode */ char *pc)
...
/*-------------------------------------------------------------------------*/
static INLINE struct svalue *
protected_rindex_lvalue (struct svalue *sp, /* TODO: opcode */ char *pc)
 ...
 * and the x-operators F_{NR,RN,RR}_RANGE_LVALUE.
 *
...
/*-------------------------------------------------------------------------*/
static INLINE struct svalue *
push_indexed_value (struct svalue *sp, /* TODO: opcode */ char *pc)
...
/*-------------------------------------------------------------------------*/
static INLINE struct svalue *
push_rindexed_value (struct svalue *sp, /* TODO: opcode */ char *pc)
...
/*-------------------------------------------------------------------------*/
static void
bad_arg_pc (int arg, int instr, struct svalue *sp, /* TODO: opcode */ char *pc)
  NORETURN;
...

{
    ...
    if (instr < 0)
    {
        ...
        /* TODO: opcode */ unsigned char *pc = inter_pc + instr;
...
/*-------------------------------------------------------------------------*/
static void
do_trace_call (/* TODO: funhdr? */ unsigned char *funstart)
...

{
    ...
    if (!++traceing_recursion) /* Do not recurse! */
    {
        ...
        memcpy(&name, funstart - 1 - sizeof name, sizeof name);
 ...
 * to function index <fx> in the current program. <fun_ix_offs> and
 * <var_ix_offs> are offsets to be added to the functions given offsets.
 *
...
/*-------------------------------------------------------------------------*/
static INLINE struct svalue *
setup_new_frame2 (/* TODO: function_ptr */ unsigned char *funstart
                 , struct svalue *sp)
    ...
    if ((i = csp->num_local_variables - num_arg) > 0)
    {
        ...
        if (num_arg < 0)
        {
            ...
            else
            {
                ...
                if (num_arg < 0)
                {
                    char *pc = funstart; /* for the ERROR() macro */
...
/*-------------------------------------------------------------------------*/
static void
eval_instruction (/* TODO: opcode */ char *first_instruction
 ...
 * a F_RETURN instruction for this call level.
 *
...
{
    register /* TODO: opcode */ char          *pc;  /* Current program pointer */
...
#   ifdef DEBUG
#       define GET_NUM_ARG \
            if (num_arg != EXTRACT_UCHAR(pc-1)) {\
                fatal("Argument count error: %d vs. %d.\n", num_arg, EXTRACT_UCHAR(pc-1));}
#   else /* DEBUG */
...
bad_right: ERRORF(("Bad right type to %s.\n", get_f_name(instruction)))
    ...
    /* --- Predefined functions accessible from LPC --- */
    ...
    CASE(F_STRING);                /* --- string <ix>          --- */
    {
         ...
         * <ix> being a 16-Bit uint, stored low byte first.
         * See also the F_CSTRINGx functions.
         */
        unsigned short string_number[2]; /* TODO: Why an array? */
    ...
    CASE(F_CLOSURE);                /* --- closure <ix>        --- */
    {
        ...
        /* TODO: uint16 */ unsigned short tmp_ushort[2]; /* TODO: Why an array? */
        /* TODO: int32 */ int ix;
    ...
    CASE(F_SYMBOL);                 /* --- symbol <ix> <num>   --- */
    {
        ...
        char *str;
        /* TODO: uint16 */ unsigned short string_number[2]; /* TODO: Why an array? */
    ...
    CASE(F_SWITCH);            /* --- switch <lots of data...> --- */
    {
        ...
        static int32 off_tab[] = {
           ...
           */
        /* TODO: opcode */ unsigned char *p0;
          /* Points after the range lookup tables (initially). */
        /* TODO: opcode */ unsigned char *p1;
          /* Points to the table of offsets. */
        /* TODO: opcode */ unsigned char *tabstart;
          /* Points to the 'v*n' table of cases */
        /* TODO: opcode */ unsigned char *end_tab;
          /* Points to the 'o*n' table of offsets for the cases */
        /* TODO: opcode */ unsigned char *break_addr;
...
#if 0
        p0 = pc; /* TODO: Delete me if nobody complains - this value is not used */
#endif
    ...
    CASE(F_TERMINAL_COLOUR);
    {
      ...
      for (j = i = 0; i < num; i++) {
         ...
         * string for it. In that case, make parts[i] a proper svalue.
         */
        ...
        if (str != NULL)
        {
            ...
            mdata = get_map_lvalue( sp->u.map, & mkey, MY_FALSE);
        }
        else
            mdata = NULL;
      ...
      *cp = 0;
      ...
      if (cp - deststr != j) {
        fatal("Length miscalculated in terminal_colour()\n"
              "    Expected: %i Was: %ld\n"
              "    String: %s\n    Indent: %i Wrap: %i\n"
             , j, (long)(cp - deststr), sp->u.string, indent, wrap);
      }
#endif
      free_svalue(sp);
    ...
    CASE(F_FILTER); /* TODO: This overlap is ugly */
    CASE(F_FILTER_ARRAY);
    {
        struct svalue *arg;
...
#define UP_TO_SVP(up)        ( \
    ...
    CASE(F_LOWER_CASE);
    {
        ...
        TYPE_TEST1(sp, T_STRING)
        for (s = sp->u.string; '\0' != (c = *s) && !isupper((unsigned char)c); s++) NOOP;
        if (c)
        {
...
/* don't use the address of u2, this would prevent putting it in a register */
    ...
    CASE(F_MAP); /* TODO: This overlap is ugly */
    CASE(F_MAP_ARRAY); {
        struct vector *res;
        struct svalue *arg;
        char *func;
...
#ifdef F_REMAINING_EVAL_TIME
          XCASE(F_REMAINING_EVAL_TIME);
          {
            push_number(max_time > 0 ? -eval_time : 1);
            break;
          }
#endif
...
not_catch:
            ...
            else if (p[0].funstart == SIMUL_EFUN_FUNSTART) {
                /* prog is undefined */
...
void count_inherits (struct program *progp)
{
    int i;
...
void count_extra_ref_in_object (struct object *ob)
{
    ob->extra_ref++;
...
static void count_extra_ref_in_closure (struct lambda *l, ph_int type)
{
        ...
        switch(p->type) {
          case T_CLOSURE:
            if (CLOSURE_MALLOCED(p->x.closure_type)) {
                ...
                l = p->u.lambda;
                if ( NULL == register_pointer(ptable, l) ) continue;
    ...
    mp_int num;
{
    ...
    for (p = svp; p < svp+num; p++) {
        switch(p->type) {
          case T_QUOTED_ARRAY:
          case T_POINTER:
            if (NULL == register_pointer(ptable, p->u.vec) ) continue;
...
void assert_master_ob_loaded()
{
    ...
    if (master_ob == 0 || master_ob->flags & O_DESTRUCTED) {
         ...
         * still in use
         * the actual freeing is done in the next backend cycle
         */
    ...
    int num_arg;
{
    ...
    switch(lsvp->x.closure_type) {
      ...
      default:
      {
        ...
        if ( (current_object = lsvp->u.ob)->flags & (O_DESTRUCTED|O_SWAPPED)) {
            if (0 != (i = current_object->flags & O_DESTRUCTED) ||
                load_ob_from_swap(current_object) < 0) 
            {
                previous_ob = csp->prev_ob;
                current_object = csp->ob;
