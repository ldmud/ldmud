 ...
 * numbers.
 *
 * TODO: Document more, like the telnet stuff.
 * TODO: Put the telnet code into a separate file?
 *---------------------------------------------------------------------------
 */
...
#undef PORTALS
  /* The Portals implementation is broken in several places.
   * TODO: Either fix the portals, or remove them.
   */
...
#define PNUMBER port_numbers[i]
    ...
    signal(SIGPIPE, SIG_IGN); /* We handle SIGPIPEs ourself */
 ...
 * restriction.
 *
 * This function also does the telnet, snooping, shadow and
 * portals handling.
 *
...

{
    ...
    /* Every new portaled message is to be prepended by ']'.
     * TODO: 'char c' is not visible here.
     */
    if (ip->out_portal)
    {
        if (dest < end)
    ...
    while(MY_TRUE)
    {
        ...
        for (; NextCmdGiver >= 0; IncCmdGiver)
        {
            ...
            if (FD_ISSET(ip->socket, &readfds)) {
                 ...
                 */
                if (ip->out_portal) {
                    socket_write(ip->portal_socket, ip->text, ip->text_end);
                    ip->text_end = 0;
                    continue;
                }
#endif /* PORTALS */
            ...
            if (ip->noecho & CHARMODE_REQ)
            {
                ...
                {
                    /* Unescaped input.
                     * Aside from the telnet negotiation (TODO: Document it)
                     * this puts the next character (addressed by
...
/* Change the IO mode <i>->noecho to the given <noecho>, performing all
