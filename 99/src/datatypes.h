...
/* T_CLOSURE secondary information. */
   ...
   * (the 'closure' instruction does to save space), but this way they
   * are easier to distinguish.
   * Note: Some code interprets these values as unsigned shorts.
   */
...
struct lambda {
    p_int ref;          /* ref count */
    struct object *ob;  /* object the closure is bound to */
    union               /* Closure information: */
    {
        unsigned short index;        /* lfun closures */
          /* (short)-1: vanished variable closure */
        char code[1];
          /* TODO: which types? Lambda closures: the function code, starting
           * with FUNCTION_TYPE.
           */
        struct lambda *lambda;
