...
void clear_ref_in_msbuf()
{
    if (sbuf)
    {
        clear_memory_reference(sbuf);
        if (sbuf->buf)
            clear_memory_reference(sbuf->buf);
    }
}

void count_ref_in_msbuf()
{
    if (sbuf)
    {
        note_malloced_block_ref(sbuf);
        if (sbuf->buf)
            note_malloced_block_ref(sbuf->buf);
    }
}

#endif /* GC_SUPPORT */
#endif /* CHECK_MAPPINGS */

#if defined(CHECK_MAPPINGS) || defined(CHECK_MAPPING_TOTAL)
/*-------------------------------------------------------------------------*/
void
int_check_map_size (mapping_t *m,  const char * tag, const char * file, int line)
{
    mp_int msize;
    ...
    if (m->condensed)
        ...
        msize = (mp_int)(m->hash ? m->hash->used - m->hash->condensed_deleted : 0);

    if (msize >= 0x10000000UL
     || (m->condensed && m->condensed->string_size < 0)
     || (m->hash && (m->hash->used >= 0x01000000L || m->hash->used < 0))
       )
    {
        char * p;

        printf("DEBUG: m %p", m); fflush(stdout);
        dump_malloc_trace(1, m);

        if (m->condensed)
        {
            p = (char *)(m->condensed) - m->condensed->misc_size * (m->num_values+1);
            printf("DEBUG: m->condensed %p", p); fflush(stdout);
            dump_malloc_trace(1, p);
        }

#ifdef TRACE_CODE
        last_instructions(50, MY_TRUE, NULL);
#endif
        if (m->condensed)
        {
            if (m->hash)
                fatal( "DEBUG: (%s-%s:%d) Illegal mapping size %ld: "
                       "map %p cond %p string %ld misc %ld, "
                       "hash %p used %ld deleted %ld\n"
                      , tag, file, line, msize, m
                      , m->condensed, m->condensed->string_size
                      , m->condensed->misc_size
                      , m->hash, m->hash->used, m->hash->condensed_deleted
                      );
            else
                fatal( "DEBUG: (%s-%s:%d) Illegal mapping size %ld: "
                       "map %p cond %p string %ld misc %ld\n"
        ...
        else
        {
            if (m->hash)
                fatal( "DEBUG: (%s-%s:%d) Illegal mapping size %ld: "
                       "map %p, hash %p used %ld deleted %ld\n"
                      , tag, file, line, msize, m
                      , m->hash, m->hash->used, m->hash->condensed_deleted
                      );
            else
...
#endif /* CHECK_MAPPINGS || CHECK_MAPPING_TOTAL */

#if defined(CHECK_MAPPINGS)
/*-------------------------------------------------------------------------*/
static void
count_cond_del (mapping_t *m UNUSED, struct condensed_mapping *cm, struct hash_mapping *hm)
{
#ifdef __MWERKS__
#    pragma unused(m)
#endif
    mp_int misc_deleted, string_deleted;

    if (!hm || !cm)
    {
        misc_deleted = string_deleted = 0;
        strbuf_addf(sbuf, "  used %ld, cond_deleted %ld, misc_deleted %ld, string_deleted %ld\n"
                        , hm->used, hm->condensed_deleted
                        , misc_deleted, string_deleted
                        );
        return;
    }
...
/*-------------------------------------------------------------------------*/
void
int_check_cond_del (mapping_t *m, struct condensed_mapping *cm, struct hash_mapping *hm, const char * tag, strbuf_t *xsbuf, const char * file, int line)
{
    mp_int misc_deleted, string_deleted;
    ...
    if (misc_deleted + string_deleted != hm->condensed_deleted)
    {
        char * p;

        if (xsbuf)
        {
            printf("DEBUG: %s\n", xsbuf->buf); fflush(stdout);
        }

        printf("DEBUG: m %p", m); fflush(stdout);
        dump_malloc_trace(1, m);

        p = (char *)(cm) - cm->misc_size * (m->num_values+1);
        printf("DEBUG: m->condensed %p", p); fflush(stdout);
        dump_malloc_trace(1, p);

        printf("DEBUG: m->hash %p", hm); fflush(stdout);
        dump_malloc_trace(1, hm);

#ifdef TRACE_CODE
        last_instructions(50, MY_TRUE, NULL);
#endif
        fatal( "DEBUG: (%s-%s:%d) cond deleted count mismatch: "
               "map %p cond %p string %ld misc %ld, "
               "hash %p used %ld deleted %ld, "
               "misc deleted %ld, string deleted %ld\n"
              , tag, file, line, m
              , cm, cm->string_size
              , cm->misc_size
              , m->hash, hm->used, hm->condensed_deleted
              , misc_deleted
              , string_deleted
              );
    }

    if (hm->condensed_deleted - misc_deleted > cm->string_size / sizeof(char*))
    {
        char * p;
...
#define count_cond_del(m,cm, hm) NOOP
...

{
    ...
    svalue_t *svp;
#ifdef DEBUG
    mp_int orig_msize; /* DEBUG: */

    check_map_size(m, "original");
    orig_msize = (mp_int)MAP_SIZE(m);
    if (orig_msize >= 0x10000000UL)
    {
        fatal("DEBUG: Illegal original mapping size (you shouldn't get this): %ld\n", orig_msize);
        return NULL;
    }
#endif
    ...
    if ( !(hm = m->hash) )
    {
...
#ifdef DEBUG
        {
            ...
            if (msize >= 0x10000000UL)
            {
                printf("DEBUG: Illegal mapping size: %ld, original %ld\n", msize+1, orig_msize);
                check_map_size(m, "create hash");
                fatal("DEBUG: Illegal mapping size: %ld, original %ld\n", msize+1, orig_msize);
                return NULL;
            }
        }
#endif
        ...
        check_cond_del(m, cm, hm, "get_value:new hash");
        return mc->data;
    }
    else
    {
         ...
         */
        check_map_size(m, "get_map_lvalue:need new hash entry");
        if (hm->used & ~hm->mask<<1)
        {
            ...
            *hm = *hm2;
            ...
            i = i ^ i >> 8;
            i &= mask;
            check_map_size(m, "get_map_lvalue:increased hash table");
        }
        ...
        mc->next = hm->chains[i];
        hm->chains[i] = mc;
        check_map_size(m, "get_map_lvalue:new hash entry");
        assign_svalue_no_free(&mc->key, map_index);
        svp = mc->data;
        for (i = num_values; --i >= 0; svp++) {
            put_number(svp, 0);
...

{
    ...
    if (cm != NULL)
    {
        ...
        for (svp = CM_MISC(cm),i = cm->misc_size; (i -= sizeof *svp) >= 0; )
        {
            ...
            if (destructed_object_ref(svp))
            {
                ...
                check_cond_del(m, cm, hm, "check_map_for_dest");
        ...
        check_map_size(m, "check_map_for_dest");
    } /* cm != NULL) */
    ...
    if ( NULL != (hm = m->hash) )
    {
        ...
        check_map_size(m, "check_map_for_destr:checking hash table");
            ...
            for (mcp2 = mcp++; NULL != (mc = *mcp2); )
            {
                ...
                if (destructed_object_ref(&(mc->key)))
                {
                    ...
                    *mcp2 = mc->next;
                    ...
                    hm->used--;
                    check_map_size(m, "check_map_for_destr:removed hash entry");
                    continue;
                }
...

{
    ...
    if (!sbuf)
    {
        sbuf = xalloc(sizeof(*sbuf));
        strbuf_zero(sbuf);
    }
    else
        sbuf->length = 0;

    strbuf_addf(sbuf, "remove_mapping from %d: map %p, index %p type (%hd:%hd) %x\n"
                    , where, m, map_index, map_index->type, map_index->x.generic, map_index->u.number);
#endif

    check_cond_del_b(m, cm, m->hash, "remove_mapping:begin");
    ...
    case T_STRING:
      {
        ...
        if (size) {
            ...
            if ( str == *(char **)key )
            {
                ...
                check_map_size(m, "remove_mapping");
                ...
                hm->condensed_deleted++;
                check_cond_del_b(m, cm, hm, "remove_mapping:string");
    ...
    case T_QUOTED_ARRAY:
      {
        ...
        keystart = keyend - size;
#ifdef CHECK_MAPPINGS
        cmstart = ((char *)cm) - cm->misc_size * (1+m->num_values);
        strbuf_addf(sbuf, "searching for type (%hd:%hd) %x\n"
                        , map_index->type, map_index->x.generic, map_index->u.number);
        strbuf_addf(sbuf, "cmstart %p, keystart %p, keyend %p\n"
                        , cmstart, keystart, keyend);
#endif
        ...
        {
#ifdef CHECK_MAPPINGS
            strbuf_addf(sbuf, "  offset %ld, key %p type (%hd:%hd) %x\n"
                            , offset, key, ((svalue_t*)key)->type, ((svalue_t*)key)->x.generic, ((svalue_t*)key)->u.number);
            if (key >= keyend || key < keystart)
            {
                fatal("DEBUG: remove_mapping:loop: key %p out of range %p..[%p..%p[, offset %ld, index_type %ld, index_x %ld, index_u %ld\n"
                     , key, cmstart, keystart, keyend
                     , (long)offset, (long)index_type, (long)index_x
                     , (long)index_u);
            }
#endif
            if ( !(u_d = (((svalue_t *)key)->u.number >> 1) -
                         (index_u >> 1)) )
            {
              if ( !(u_d = ((svalue_t *)key)->x.generic - index_x) )
                if ( !(u_d = ((svalue_t *)key)->type - index_type) )
                {
                    ...
                    /* Count the deleted entry in the hash part.
                     * Create it if necessary.
                     * For debugging only, we do this before everything else.
                     */
                    if ( !(hm = m->hash) )
                    {
                        hm = (struct hash_mapping *)xalloc(sizeof *hm);
                    ...
                    if (key >= keyend || key < keystart)
                    {
                        fatal("DEBUG: remove_mapping:found1: key %p out of range %p..[%p..%p[, offset %ld, index_type %ld, index_x %ld, index_u %ld\n"
                             , key, cmstart, keystart, keyend
                             , (long)offset, (long)index_type, (long)index_x
                             , (long)index_u);
                    }
                    ...
                    free_svalue( (svalue_t *)key );
                      /* Note: clobbers .type */
                    check_cond_del_b(m, cm, hm, "remove_mapping:found1");

                    svp = (svalue_t *)
                      (keystart - ( num_values * (keyend - key) ) );
                    for (i = num_values; --i >= 0 ;svp++) {
#ifdef CHECK_MAPPINGS
                        if ((char*)svp >= keystart || (char*)svp < cmstart)
                        {
                            fatal("DEBUG: remove_mapping:found2: svp %p out of range [%p..%p[..%p, i %d\n"
                                 , svp, cmstart, keystart, keyend
                                 , i
                                 );
                        }
#endif
                        free_svalue(svp);
                        put_number(svp, 0);
                        check_cond_del_b(m, cm, hm, "remove_mapping:found2");
                    }
...
#ifdef CHECK_MAPPINGS
                        if (key >= keyend-sizeof(svalue_t) || key < keystart)
                        {
                            fatal("DEBUG: remove_mapping:move1: key %p out of range %p..[%p..%p[, offset %ld, index_type %ld, index_x %ld, index_u %ld\n"
                                 , key, cmstart, keystart, keyend
                                 , (long)offset, (long)index_type, (long)index_x
                                 , (long)index_u);
                        }
#endif
#ifdef CHECK_MAPPINGS
                        strbuf_addf(sbuf, "key %p := key+1 %p type (%hd:%hd) %x\n"
                                        , key, (svalue_t *)key+1
                                        , ((svalue_t*)key+1)->type
                                        , ((svalue_t*)key+1)->x.generic
                                        , ((svalue_t*)key+1)->u.number);
#endif /* CHECK_MAPPINGS */
                        *((svalue_t *)key) = *((svalue_t *)key+1);
                        count_cond_del(m, cm, hm);
                        key += sizeof(svalue_t);
                        check_cond_del_b(m, cm, hm, "remove_mapping:move1");
                        svp2 = svp - num_values;
                        for (i = num_values; --i >= 0 ;svp++, svp2++)
                        {
#ifdef CHECK_MAPPINGS
                            if ((char*)svp >= keystart || (char*)svp < cmstart
                             || (char*)svp2 >= keystart || (char*)svp2 < cmstart
                               )
                            {
                                fatal("DEBUG: remove_mapping:move2: svp %p, svp2 %p out of range [%p..%p[..%p, i %d\n"
                                     , svp, svp2, cmstart, keystart, keyend
                                     , i
                                     );
                            }
#endif
                            *svp2 = *svp;
                            put_number(svp, 0);
                            check_cond_del_b(m, cm, hm, "remove_mapping:move2");
                        }
                    }
                    ...
                    if (key >= keyend || key < keystart)
                    {
                        fatal("DEBUG: remove_mapping:invalidate: key %p out of range %p..[%p..%p[, offset %ld, index_type %ld, index_x %ld, index_u %ld\n"
                             , key, cmstart, keystart, keyend
                             , (long)offset, (long)index_type, (long)index_x
                             , (long)index_u);
                    }
#endif
#ifdef CHECK_MAPPINGS
                    strbuf_addf(sbuf, "invalidating key %p type (%hd:%hd) %x\n"
                                    , key
                                    , ((svalue_t*)key)->type
                                    , ((svalue_t*)key)->x.generic
                                    , ((svalue_t*)key)->u.number);
#endif
                    ((svalue_t *)key)->type = T_INVALID;
                    count_cond_del(m, cm, hm);

#ifndef CHECK_MAPPINGS
                    /* Count the deleted entry in the hash part.
                     * Create it if necessary.
                     */
                    if ( !(hm = m->hash) )
                    ...
                    hm->condensed_deleted++;
#endif
                    check_cond_del_b(m, cm, hm, "remove_mapping:misc");
                    return;
                }
            }
...

{
    ...
    check_cond_del(m, m->condensed, m->hash, "copy_mapping:orig");
    ...
    *cm2 = *cm;
    ...
    m2->ref = 1;
    check_map_size(m2, "copy_mapping");
    check_cond_del(m2, m2->condensed, m2->hash, "copy_mapping:result");
...

{
    ...
    check_cond_del(m, m->condensed, m->hash, "resize_mapping:orig");
    ...
    *cm2 = *cm;
    ...
    m2->ref = 1;
    check_map_size(m2, "resize_mapping");
    check_cond_del(m2, m2->condensed, m2->hash, "resize_mapping:result");
...

{
...
#ifdef DEBUG
    check_map_size(m1, "add:original 1");
    check_cond_del(m1, m1->condensed, m1->hash, "add:orig 1");
    check_map_size(m2, "add:original 2");
    check_cond_del(m2, m2->condensed, m2->hash, "add:orig 2");
#endif
...
#ifdef DEBUG
    {
        check_map_size(m1, "add:main str merged 1");
        check_map_size(m2, "add:main str merged 2");
        if (string_size != cm3->string_size)
        {
            fatal("DEBUG: (main strings merged) string_size3 clobbered: %ld, original %ld\n", cm3->string_size, string_size);
            return NULL;
        }
        if (misc_size != cm3->misc_size)
        {
            fatal("DEBUG: (main strings merged) misc_size3 clobbered: %ld, original %ld\n", cm3->misc_size, misc_size);
            return NULL;
        }
    }
#endif
...
#ifdef DEBUG
    {
        check_map_size(m1, "add:other str merged 1");
        check_map_size(m2, "add:other str merged 2");
        if (string_size != cm3->string_size)
        {
            fatal("DEBUG: (remaining strings merged) string_size3 clobbered: %ld, original %ld\n", cm3->string_size, string_size);
            return NULL;
        }
        if (misc_size != cm3->misc_size)
        {
            fatal("DEBUG: (remaining strings merged) misc_size3 clobbered: %ld, original %ld\n", cm3->misc_size, misc_size);
            return NULL;
        }
    }
#endif
...
#ifdef DEBUG
    {
        check_map_size(m1, "add:main misc merged 1");
        check_map_size(m2, "add:main misc merged 2");
        if (string_size != cm3->string_size)
        {
            fatal("DEBUG: (main misc merged) string_size3 clobbered: %ld, original %ld\n", cm3->string_size, string_size);
            return NULL;
        }
        if (misc_size != cm3->misc_size)
        {
            fatal("DEBUG: (main misc merged) misc_size3 clobbered: %ld, original %ld\n", cm3->misc_size, misc_size);
            return NULL;
        }
    }
#endif
...
#ifdef DEBUG
    {
        check_map_size(m1, "add:other misc merged 1");
        check_map_size(m2, "add:other misc merged 2");
        if (string_size != cm3->string_size)
        {
            fatal("DEBUG: (remaining misc merged) string_size3 clobbered: %ld, original %ld\n", cm3->string_size, string_size);
            return NULL;
        }
        if (misc_size != cm3->misc_size)
        {
            fatal("DEBUG: (remaining misc merged) misc_size3 clobbered: %ld, original %ld\n", cm3->misc_size, misc_size);
            return NULL;
        }
    }
#endif
...
#ifdef DEBUG
    {
        check_map_size(m1, "add:result allocated 1");
        check_map_size(m2, "add:result allocated 2");
        if (string_size != cm3->string_size)
        {
            fatal("DEBUG: (result allocated) string_size3 clobbered: %ld, original %ld\n", cm3->string_size, string_size);
            return NULL;
        }
        if (misc_size != cm3->misc_size)
        {
            fatal("DEBUG: (result allocated) misc_size3 clobbered: %ld, original %ld\n", cm3->misc_size, misc_size);
            return NULL;
        }
        check_map_size(m3, "add:result allocated 3");
    }
#endif
    ...
    check_cond_del(m3, m3->condensed, m3->hash, "add:result allocated");
...
#ifdef DEBUG
    {
        check_map_size(m1, "add:result adjusted 1");
        check_map_size(m2, "add:result adjusted 2");
        if (string_size != cm3->string_size)
        {
            fatal("DEBUG: (result adjusted) string_size3 clobbered: %ld, original %ld\n", cm3->string_size, string_size);
            return NULL;
        }
        if (misc_size != cm3->misc_size)
        {
            fatal("DEBUG: (result adjusted) misc_size3 clobbered: %ld, original %ld\n", cm3->misc_size, misc_size);
            return NULL;
        }
        check_map_size(m3, "add:result adjusted 3");
    }
#endif
...
#ifdef DEBUG
    {
        check_map_size(m1, "add:hash1 copied 1");
        check_map_size(m2, "add:hash1 copied 2");
        if (string_size != cm3->string_size)
        {
            fatal("DEBUG: (hash1 copied) string_size3 clobbered: %ld, original %ld\n", cm3->string_size, string_size);
            return NULL;
        }
        if (misc_size != cm3->misc_size)
        {
            fatal("DEBUG: (hash1 copied) misc_size3 clobbered: %ld, original %ld\n", cm3->misc_size, misc_size);
            return NULL;
        }
        check_map_size(m3, "add:hash1 copied 3");
    }
#endif
...
#ifdef DEBUG
    {
        check_map_size(m1, "add:hash2 copied 1");
        check_map_size(m2, "add:hash2 copied 2");
        if (string_size != cm3->string_size)
        {
            fatal("DEBUG: (hash2 copied) string_size3 clobbered: %ld, original %ld\n", cm3->string_size, string_size);
            return NULL;
        }
        if (misc_size != cm3->misc_size)
        {
            fatal("DEBUG: (hash2 copied) misc_size3 clobbered: %ld, original %ld\n", cm3->misc_size, misc_size);
            return NULL;
        }
        check_map_size(m3, "add:hash1 copied 3");
    }
#endif
    check_cond_del(m3, m3->condensed, m3->hash, "add:result");
...
#ifdef DEBUG
    check_map_size(m, "walk_mapping:after string walk");
#endif
...
#ifdef DEBUG
    check_map_size(m, "walk_mapping:after misc walk");
#endif
...
#ifdef DEBUG
    check_map_size(m, "walk_mapping:after chain walk");
#endif
        } while (--size >= 0);
#ifdef DEBUG
    check_map_size(m, "walk_mapping:after hash walk");
#endif
    }
...

{
    ...
    while (--num >= 0)
    {
        ...
        check_cond_del(m, m->condensed, m->hash, "compact:setup");
        ...
        check_map_size(m, "compact:mergesort string");
        ...
        check_map_size(m, "compact:mergesort misc");
         ...
         */
        {
            ...
            check_map_size(m, "compact:mergesort1");
            ...
            if (string_total < 0)
            {
                printf("DEBUG: compact %p -> condensed %p: string %ld, misc %ld\n", m, cm, cm->string_size, cm->misc_size);
                printf("DEBUG:   string_used %ld, misc_used %ld, cond_deleted %ld, misc_deleted %ld\n", string_used, misc_used, hm->condensed_deleted, misc_deleted);
                printf("DEBUG:   string_total %ld, misc_total %ld\n", string_total, misc_total);
                fflush(stdout);
            }
#endif
        ...
        m->condensed = cm2;
        m->hash = NULL;
        check_cond_del(m, m->condensed, m->hash, "compact:done");
        check_map_size(m, "compact:done");
...
void
m_check_total_mapping_size (Bool bFatal, const char * file, int line)
...

{
    ...
    if (last_size_ok && !this_size_ok)
    {
        ...
        {
            fatal("DEBUG: (%s : %ld) Invalid total mapping size %ld"
#ifdef MALLOC_smalloc
                  " (avail %ld)"
#endif
                  ", was %ld\n"
                 , file, (long)line, (long)total
#ifdef MALLOC_smalloc
                 , (long)available
#endif
                 , (long)last_size);
...
#ifdef CHECK_MAPPINGS
        remove_mapping(m, first_hairy, 2);
#else
...

{
    ...
    dprintf3(gcollect_outfd, "DEBUG: map '%s' %d (num values %d)"
                           , (p_int)(m->user->name ? m->user->name : "<0>")
    ...
    if ((cm = m->condensed) != NULL)
    {
        ...
        total += subtotal;
#if 0 && defined(CHECK_MAPPING_TOTAL)
        dprintf3(gcollect_outfd, " + %d (string %d, misc %d)"
                               , (p_int)subtotal
                               , (p_int)cm->string_size
                               , (p_int)cm->misc_size
                               );
#endif
    }
...
#ifdef DEBUG
    check_map_size((mapping_t *)extra, "add_to_mapping_filter before");
#endif
...
#ifdef DEBUG
    check_map_size((mapping_t *)extra, "add_to_mapping_filter after");
#endif
}
...
#ifdef DEBUG
    check_map_size(m1, "add_to_mapping 1");
    check_map_size(m2, "add_to_mapping 2");
#endif
...
#endif
#ifdef CHECK_MAPPINGS
    remove_mapping((mapping_t *)extra, key, 3);
#else
