 ...
 *    the called function gets exactly as many arguments as it wants; if
 *    necessary, the actual function call will remove excessive arguments
 *    or push '0's for missing arguments. The number of arguments will be
 *    stored in the control stack, so that the return instruction not
 *    needs to know it explicitely.
 *
 ...
 *    returned value.
 *
 *    When a function returns, it will use the instruction F_RETURN, which
...
static void free_protector_svalue(struct svalue *);
...
static void assign_string_range(struct svalue *source, int do_free);
static void assign_protected_string_range(
    struct protected_range_lvalue *dest,struct svalue *source, int do_free);
