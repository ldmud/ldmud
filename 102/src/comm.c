...
static Bool send_erq(int handle, int request, char *arg, int arglen);
...
#ifdef DEBUG
    if (d_flag > 1)
        debug_message("[%s(%ld)]: %s", command_giver->name,
...
#endif
        if (n != chunk)
            fprintf(stderr, "write socket: wrote %ld, should be %ld.\n",
                    (long)n, (long)chunk);
...

{
    ...
    new_interactive->prompt.x.string_type = STRING_VOLATILE;
...
/*-------------------------------------------------------------------------*/
static Bool
set_call (struct object *ob, struct input_to *it, Bool noecho)
...

{
    ...
    promptp->x.string_type = STRING_VOLATILE;
