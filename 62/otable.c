 ...
 * The Reset Table:
 *
 *   This table keeps all objects which require a reset in a list of lists.
 *   The objects with reset times within the same RESET_GRANULARITY interval
 *   are organized in one list; the master list is made up from these
 *   list sorted by ascending time. Objects with next reset time of 0 
 ...
 * unless it's 0.
 * <obj> must not be part of the table.
 */
...
printf("DEBUG: rtable_remove('%s' due %ld) cur %ld\n", obj->name, obj->time_reset, current_time);
    ...
    if (!pr || pr->duetime != duetime)
    {
        fatal("rtable_remove() entry for object '%s' not found.\n", obj->name);
        /* NOTREACHED */
        return;
    }
#endif
    ...
    if (!ob || ob != obj)
    {
        fatal("rtable_remove() object '%s' not found.\n", obj->name);
        /* NOTREACHED */
        return;
    }
#endif
...

{
    ...
    duetime = (cur_time + RESET_GRANULARITY - 1) / RESET_GRANULARITY;

    /* Find the proper entry for this object */
    for (prev_e = NULL, pr = reset_table; pr && duetime >= pr->duetime; )
    {
        prev_e = pr;
        pr = pr->next;
    }
...

{
    if (verbose)
    {
        ...
        overdue = 0;
        due = 0;
        for (pr = reset_table; pr && pr->duetime < duetime; pr = pr->next)
            overdue += pr->num;
        if (pr && pr->duetime == duetime)
            due = pr->num;
        ...
        if (rtable_num_entries)
        {
            ...
            add_message("%ld objects due at %ld times: avg. %s objs per time.\n"
                       , rtable_num_objs, rtable_num_entries, sbuf);
    ...
    add_message("reset table overhead\t\t\t %8d\n",
