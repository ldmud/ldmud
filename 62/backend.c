...
{
    struct object *obj;  /* Current object reset */
...
printf("DEBUG: %ld RESET '%s'%s due %ld\n"
      ...
      , obj->time_reset
            ...
            if (!(obj->flags & O_DESTRUCTED) && obj->time_reset)
                rtable_add(obj);
        }
...
static struct object *next_ob;
   ...
   */
    ...
    for (; NULL != (ob = next_ob); ) {
...
#else
        /* Check if a reset() is due. If that is the case, the object will
         * not be cleant up.
         * We do nothing about the actual reset itself.
         */
        if (ob->time_reset 
         && ob->time_reset < current_time && !(ob->flags & O_SWAPPED))
        {
            NOOP;
        }
#endif
        ...
        /* TODO: Maybe move this code into the reset function? Would
         * TODO:: make more sense there.
