 ...
 * a blueprint, and "/std/thing#45" for a clone. However, this name
 * can be changed with the efun rename_object().
 *
 ...
 * (in our example "/std/thing") and can't be changed. However, for virtual
 * objects .load_name is the virtual name - the real program name is
 * .prog->name.
 *
 ...
 *
 * .next_all and .next_hash are used to store the object. .next_all
 * is the link pointer in the list of all objects, .next_hash is the
 * link pointer in the object table (see otable.c).
 *
...
/*-------------------------------------------------------------------------*/
static void
register_mapping_filter (struct svalue *key, struct svalue *data, char *extra)
...

{
    ...
    struct pointer_record *pointer_table_space[256];
      /* Table of arrays and mappings, to recognize shared usages.
       */
    size_t len;
    int i;
    int f;
