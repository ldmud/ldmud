 ...
 *                    |  Break addresses for switch cases.
 *                    |  ...
 *                    |  Local variable number 1
 *                    |  Local variable number 0
 *                    |  ...
 *                    |  Argument number 1
 *    frame_pointer-> |  Argument number 0
 *                    |
 *                    |
 *    start_of_stack  -----
 *
...
v*n: case values, char * or int, host byte order
...
#define SET_TRACE_EXEC (trace_level & TRACE_EXEC && eval_cost < 0 && \
  ...
  /* If TRACE_EXEC is requested is not active yet (eval ticks negative),
...
static char **current_strings;
  /* Pointer to the string block of the current program.
   */

struct svalue *current_variables;
  /* Pointer to begin of current variable block */
...
static struct svalue *old_hooks = NULL;
  /* Array of multiple of NUM_CLOSURE_HOOKS entries holding all the old
   * driver hook closures replaced during this and the previous execution
   * threads. The closures are not freed immediately on replacement in case
   * they are still used. Instead, the backend frees them explicitely.
   */

static int num_old_hooks = 0;
  /* The current number of entries in <old_hooks>, given as multiples
   * of NUM_CLOSURE_HOOKS.
   */
...
/* don't use the address of u2, this would prevent putting it in a register */
    ...
    CASE(F_STRING);
    {
        ...
        pc += 2;
        /* amylaar: STRING_VOLATILE is no good here, because strings are
...
free_old_driver_hooks(void)
{
    ...
    for (i = NUM_CLOSURE_HOOKS * num_old_hooks; i--;) 
    {
...
void assert_master_ob_loaded()
{
    ...
    if (master_ob == 0 || master_ob->flags & O_DESTRUCTED) {
        ...
        {
            int i;
            int sz = NUM_CLOSURE_HOOKS * sizeof(struct svalue);
            struct svalue *new;
            ...
            {
                new = (struct svalue*)
                                rexalloc(old_hooks, (num_old_hooks+1) * sz);
                if (new) {
                    old_hooks = new;
                    new = old_hooks + num_old_hooks * NUM_CLOSURE_HOOKS;
                    num_old_hooks++;
                }
            }
            
            if (new)
                memcpy(new, closure_hook, sz);
