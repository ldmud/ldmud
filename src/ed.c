...
# define add_error(s)        { add_message(s, lineno); errs++; return; }
...

{
    ...
    if (*text != '\0') /* don't shift lines with ws only */
    {
        ...
        add_error("Result of shift would be too long, line %d\n");
    }
}
    ...
    while (*p != '\0') {
        ...
        else if (quote != '\0')
        {
            ...
            for (;;)
            {
                ...
                {
                    add_error("Unterminated string in line %d\n");
                }
                else if (*p == '\\' && *++p == '\0')
                {
                    break;
            ...
            case '#':
                if (*p == '\'')
                {
                    ...
                    else
                    {
                        ...
                        {
                            add_error("Missing function name after #' in line %d\n");
                        }
                        p = end;
                    }
                    token = TOKEN;
        ...
        for (;;)
        {
            ...
            if (f[top] <= g[token])     /* shift the token on the stack */
            {
                ...
                {
                    /* out of stack */
                    add_error("Nesting too deep in line %d\n");
                }
    ...
    case 'M':
      {
        ...
        pat = REGCOMP((unsigned char *)"\015$"
                     , P_EXCOMPAT, MY_TRUE);
        nchng = subst(pat, "", MY_FALSE, MY_FALSE);
        REGFREE(pat);
