    ...
    if (unused_size < size)
    {
...
#if defined(DEBUG_SMALLOC_ALLOCS)
        {
            ...
            dprintf2(gcollect_outfd, "(%d / %d left)\n"
                    , (unused_size >= T_OVERHEAD * SINT)
                      ? ((p_int)unused_size - T_OVERHEAD * SINT) : 0
                    , (p_int)unused_size);
        ...
        if (unused_size)
        {
            if (unused_size < SINT + T_OVERHEAD*SINT)
            {
