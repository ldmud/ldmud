    ...
    if (pat && hs_regexec(pat,curr,start)) {
        do {
            ...
            if (subclosure != NULL)
            {
                ...
                /* Put references to the allocated memory onto the stack
                 * so that a runtime error can clean them up.
                 */
                push_malloced_string(buf);
                push_malloced_string(match);
                ...
                /* Remove the cleanup references to the buffers
                 * from the stack, but don't free the memory.
                 */
                inter_sp -= 2;
