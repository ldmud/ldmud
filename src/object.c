            ...
            case CLOSURE_LFUN:
              {
...

                {
                    ...
                    if (v->u.lambda->ob == current_object)
                    {
                        ...
                        /* For inherited lfun closures, add the
                         * '-<inheritname>' */
                        if (l->function.lfun.inhIndex)
                        {
                            char * source = inhProg->name;
                            L_PUTC('-');
                            c = *source++;
                            do L_PUTC(c) while ( '\0' != (c = *source++) && '.' != c);
                        }
            ...
            case CLOSURE_IDENTIFIER:
              {
                ...
                l = v->u.lambda;
                if (l->function.lfun.index == VANISHED_VARCLOSURE_INDEX)
                {
                    rc = MY_FALSE;
                    break;
                }
                ...
                source = l->ob->prog->variable_names[l->function.lfun.index].name;
            ...
            default:
                if (type < 0)
                {
                    ...
                    case CLOSURE_OPERATOR:
                      {
                        /* For all operators whose internal instruction name
                         * differs from the external representation, give
                         * the re-parseable name.
                         */
                        char *s = NULL;
                    ...
                    case CLOSURE_EFUN:
                      {
                        ...
                        /* For all efuns whose internal instruction name
                         * differs from the external representation, give
                         * the re-parseable name.
                         */
        ...
        case '#': /* A closure: skip the header and restart this check
                   ...
                   */
          {
            ...
            pt = &pt2[1];
               ...
               * We also know that there won't be a string or shared object
               * be following.
               */
            ...
            case 'e': /* An efun closure */
              {
                ...
                else if (pp && pp->type == I_TYPE_GLOBAL)
                {
            ...
            case 's': /* A sefun closure */
              {
                ...
                /* Did we find a suitable identifier? */
                if (pp
                 && pp->type == I_TYPE_GLOBAL
                 && pp->u.global.sim_efun >= 0)
                {
                    svp->type = T_CLOSURE;
                    svp->x.closure_type
