...
#define WARNF(s) {inter_pc = pc; inter_sp = sp; warnf s ;}
 ...
 */
{
    ...
    if (prog != obj->prog
     ...
     && (prog->variable_names[inheritp->variable_index_offset
       ...
       )
    {
        ...
        while (i)
        {
            if (inh->prog == inheritp->prog
             && obj->prog->variable_names[inh->variable_index_offset
...

{
    ...
    for (svp = VALUE_STACK; svp <= inter_sp; svp++)
    {
        ...
        /* Alien-Lfun closures need a second check */
        if (svp->type == T_CLOSURE
         && CLOSURE_MALLOCED(type = svp->x.closure_type)
...
#define MY_LAMBDA_VALUE_OFFSET (sizeof(svalue_t) + \
	 ...
	 * If the object is given as its filename, the driver
        ...
        {
            tell_object(ob, sp->u.string);
            free_svalue(sp);
        }
        else
        {
            apply(STR_CATCH_MSG, ob, 1);
              /* Will pop the array from the stack. */
        }
    ...
    CASE(F_PRESENT);                /* --- present             --- */
      {
        ...
        ob = e_object_present(arg, ob
                             , num_arg > 1, (num_arg > 1) ? arg[1].u.number : 0);
            ...
            case T_CLOSURE:
              {
                ...
                char * cstr = closure_to_string(sp, MY_FALSE);
                free_svalue(sp);
                put_malloced_string(sp, cstr);
                break;
              }
