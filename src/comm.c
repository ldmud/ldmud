...
#   define IncCmdGiver    NextCmdGiver--
    ...
    while(MY_TRUE)
    {
        ...
        for (; NextCmdGiver >= 0; IncCmdGiver)
        {
            ...
            if (
#ifndef SIMULATE_CHARMODE
                (ip->noecho & (CHARMODE_REQ|CHARMODE)) == (CHARMODE_REQ|CHARMODE)
#else
                (ip->noecho & (CHARMODE_REQ|CHARMODE))
#endif
               )
            {
                DT(("'%s' CHARMODE_REQ\n", ip->ob->name));
...
/*-------------------------------------------------------------------------*/
void
set_noecho (interactive_t *i, char noecho, Bool local_change)
...

{
    ...
    if (!i->tn_enabled)
    {
        DT(("'%s' set_noecho(%02x) old %02x: TN disabled\n"
           , i->ob->name, noecho, i->noecho));
    }
    ...
    DT(("'%s' set_noecho(%02x%x) old %02x %s\n"
       , i->ob->name, local_change ? " local" : "", noecho, old, decode_noecho(old)));
    DT(("'%s'   -> confirm: %02x %s\n"
    ...
    if (((confirm ^ old) & (NOECHO_MASK|CHARMODE_MASK)) && i->tn_enabled)
    {
        DT(("'%s' set_noecho():   Mode change\n", i->ob->name));
        ob = i->ob;
           ...
           )
        {
            ...
            push_number(noecho);
            push_valid_ob(ob);
            push_number(local_change ? 1 : 0);
            if (driver_hook[H_NOECHO].type == T_STRING)
                secure_apply(driver_hook[H_NOECHO].u.string, ob, 3);
            else
            {
                 if (driver_hook[H_NOECHO].x.closure_type == CLOSURE_LAMBDA)
                     driver_hook[H_NOECHO].u.lambda->ob = ob;
                secure_call_lambda(&driver_hook[H_NOECHO], 3);
            }
...
#ifdef USE_TLS
                if (ip->tls_status != TLS_INACTIVE)
                    wrote = (mp_int)tls_write(ip, message, (size_t)size);
                else
#endif /* USE_TLS */
    ...
    case CHARSET_STRING:
      {
        ...
        xallocate(rc, length+1, "result string");
        rc[length] = '\0';
...

{
    ...
    if ( sp[-1].type != T_STRING
     ...
     && (sp[-1].type != T_NUMBER || sp[-1].u.number != 0)
       )
    {
        bad_xefun_arg(1, sp);
        /* NOTREACHED */
...

{
    ...
    if (privilege_violation4("set_max_commands", sp->u.ob, NULL, num, sp))
        ip->maxNumCmds = num;
...

{
    ...
    rc = (ip->tn_enabled != 0);
    if (num >= 0
     && privilege_violation4("enable_telnet", sp->u.ob, NULL, num, sp))
        ip->tn_enabled = (num != 0);
...

{
    ...
    if (!privilege_violation4("net_connect", NULL, host, port, sp))
    {
        sp = pop_n_elems(2, sp);
        sp++;
        put_number(sp, -1);
