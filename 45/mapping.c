 ...
 * TODO: document me.
 *
 * The module uses (.u.number, .x.generic, .type) as sort/search key
 * in the condensed-misc part.
 *---------------------------------------------------------------------------
 */
...

{
    ...
    /* Scan the non-string-key values in the condensed part,
...

{
    ...
    *cm2 = *cm;
    ...
    /* Copy the non-string keys and their associated values */
...

{
    ...
    /* Merge the non-string-keyed entries.
...

{
    ...
    /* Walk the condensed non-string-key entries.
     */
 ...
 * separated into string and non-string key entries and sorted, then
...

{
    ...
    while (--num >= 0)
    {
        ...
        mp_int string_used, misc_used;
          /* Number of string/non-string entries in the hash */
        ...
        struct map_chain *string_hook2, *misc_hook2;
          /* All string-/non-string-keyed entries in two long chains.
         ...
         * keyed entries, two for non-string keyed. These two chains
        ...
        /* Add the remaining odd elements */
        if (last_string) {
            last_string->next = string_hook1;
        ...
        /* --- Mergesort the non-string-key entries ---
         *
...
static void f_walk_mapping_cleanup(arg)
    struct svalue *arg;
{
    struct svalue *svp;
    struct mapping *m;
...
static struct svalue *walk_mapping_prologue(m, sp)
    struct mapping *m;
    struct svalue *sp;
{
    struct hash_mapping *hm;
