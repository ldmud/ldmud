...
case 255:
	...
	*sp = *svp;	/* This way, the same ref counts are maintained */
    ...
    CASE(F_TERMINAL_COLOUR);
    {
      ...
      int wrap = 0;
      int indent = 0;
      int max_string_length = 200000; /* sorta hack, MudOS has that defable */
      struct svalue * mdata;
      struct svalue mkey;
...
/*	    debug_message(
	...
	*sp = *arg;	   /* Re-insert function result */
    ...
    {
	inter_pc = pc;
	find_call_out(current_object, sp, 1);
	break;
    }
    CASE(F_FIND_CALL_OUT);
    {
	inter_pc = pc;
	find_call_out(current_object, sp, 0);
	break;
    }
    CASE(F_WRITE);
	assign_eval_cost();
...
int apply_cache_shared = 0, apply_cache_noshared = 0, apply_cache_null = 0; /* TODO: delete me when evaluated */
#endif
    ...
    int num_arg;
{
	...
	while ((shadow = O_GET_SHADOW(ob)->shadowed_by) &&
...
retry_for_shadow:
	...
	/* Heuristic to find out if fun is no shared string,
         ...
         * not exist anywhere. As a side effect, any non-shared fun
         * is made shared. */
	fun = findstring(fun);
	if (!fun)
	    goto failure2;
...
#ifdef APPLY_CACHE_STAT
	apply_cache_miss++;
        /* TODO: If at this point fun is really always a shared string,
         * TODO:: the call to findstring() could be omitted altogether.
         * TODO:: Then this piece of code can go as well.
         */
        if (cache_name[ix] != fun)
        {
          char * shx;
          shx = findstring(fun);
          if (shx == fun)
    ...
    int num_arg;
{
	...
	if ( (current_object = l->ob)->flags & (O_DESTRUCTED|O_SWAPPED)) {
	    ...
	    if ((d = current_object->flags & O_DESTRUCTED) ||
