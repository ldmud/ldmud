...
{
  fputs("LDMud " GAME_VERSION LOCAL_LEVEL 
    ...
    if (cUnknown == eOption)
    {
      if (bShort) /* search the short option */
      {
        for (iOption = 0; iOption < sizeof(aShortOpts) / sizeof(aShortOpts[0]); iOption++)
        {
          ...
          {
            eOption = aShortOpts[iOption].eNumber;
      ...
      { 
        for (iOption = 0; iOption < sizeof(aLongOpts) / sizeof(aLongOpts[0]); iOption++)
          if (!strncasecmp(pArg, aLongOpts[iOption].pOption, iArglen))
          {
            eOption = aLongOpts[iOption].eNumber;
