...
static /* TODO: BOOL */ int did_swap;
   ...
   */
          ...
          )
    {
        ...
        /* TODO: BOOL */ short bResetCalled;  /* TRUE: reset() called */
        /* TODO: BOOL */ short bWasSwapped;  /* TRUE: obj was swapped out */
        ...
        bWasSwapped = (obj->flags & O_SWAPPED) ? MY_TRUE : MY_FALSE;
         ...
         && obj->time_reset < current_time)
        {
            ...
            else if (!did_reset || !comm_time_to_call_heart_beat)
            {
                ...
                /* If reset() was called in a then swapped-out object, we
                 * adjust the obj->time_of_ref so that the object will be
                 * swapped out real soon - but not immediately, as the call
                 * may have triggered new activity.
                 * 'real soon' means in this case 0.5*min(reset_delay,
                 * swap_delay).
                 *
                 * If the object was not swapped out, we simply restore the
                 * old time_of_ref.
                 */
                if (bWasSwapped)
                {
                    int delay = INT_MAX;
                    int reset_interval = obj->time_reset - current_time;
            
                    if (reset_interval < 0) /* Object is not reset at all */
                        reset_interval = INT_MAX;

                    if (time_to_swap && time_to_swap < delay)
                        delay = time_to_swap;
                    if (time_to_swap_variables && time_to_swap_variables < delay)
                        delay = time_to_swap_variables;
                    if (reset_interval && reset_interval < delay)
                        delay = reset_interval;
                    if (INT_MAX != delay)
                        obj->time_of_ref = current_time - (delay / 2);
                /* else the time_of_ref doesn't matter anyway */
                }
                else
          ...
          && (obj->flags & O_WILL_CLEAN_UP))
        {
            ...
            if (closure_hook[H_CLEAN_UP].type == T_CLOSURE)
            {
                ...
                obj->time_of_ref = current_time;
                  /* in case the closure doesn't touch the object itself */
