...
fprintf(stderr, "DEBUG: closure:   read branch %p -> %p (offset %lx), q-p %d, i %ld, growth %ld\n"
              , current.code+start, current.code+start+offset, offset, q-p, i, growth);
				end++;
				void_dest++;
				non_void_dest++;
				if (offset > 0x7fff)
		    ...
		    *current.codep = F_SWITCH - F_OFFSET;
			...
			for (; j--; labels++) {
			    ...
			    if (j && labels[1].type == T_CLOSURE &&
                                  /* TODO: gcc says the following comparison
                                   * is always zero (or does it mean the one
                                   * above by this?).
                                   */
