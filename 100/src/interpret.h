...
struct control_stack {
    ...
    int   extern_call;
      /* Flag if evaluator should return. It is set for every intra-object
       * call (which might be a call to the current_object) and therefore
       * denotes those entry where the this_object/previous_object changes.
       * It is or'ed with CS_PRETEND if pretend_to_be is set.
       */
