 ...
 * and TIME_TO_RESET. However, the object is not entered/removed from
 * the reset table if <arg> is H_RESET.
...
#ifndef OLD_RESET
    /* If its one of the create calls, add the object to the rtable
     */
    if (arg != H_RESET && !(ob->flags & O_DESTRUCTED) && ob->time_reset)
        rtable_add(ob);
#endif
} /* reset_object() */
